...
def visualize_model(data: DataLoader, model: alexnet, device: str, num_images=6):
    """Plot test data"""
...
if __name__ == "__main__":
    ...
    #num_ftrs = model_cp.classifier[-1].in_features  # for alexnet
    #model_cp.fc = nn.Linear(num_ftrs, 2)

    # ResNet
    #model_cp = inception_v3(init_weights=True) #resnet18(weights='IMAGENET1K_V1')  #
    #model_cp.aux_logits = False
    num_ftrs = model_cp.fc.in_features  # for resnet and inception
    model_cp.fc = nn.Linear(num_ftrs, 2)  # for resnet and inception
    ...
    data_transforms = {
        'val': Compose([Resize(256),
                        CenterCrop(224),  # 224 for alexnet and resnet18 / 299 for inception v3
                        ToTensor()]),
    }
    ...
    #plt.savefig(f"results/{RESULTS}.jpg")
